  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({
        ...
        suggestion:
          "Go to http://localhost:5173/signup to create an account first",
      });
    }
    ...
    await user.save();
    console.log(
      `✅ Updated user ${user.name} - Tokens: ${user.tokenBalance}, Verified: ${user.isVerified}`,
    );
    ...
    const sessionData = {
      ...
      description:
        "A live session where you can join right now! We will cover React fundamentals, hooks, and best practices for state management.",
    ...
    res.status(201).json({
      success: true,
      message:
        "Test sessions created successfully! You now have a JOINABLE session and a COMPLETED session with AI summary.",
      data: {
        teacher: {
          id: teacher._id,
          name: teacher.name,
